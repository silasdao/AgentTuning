...
class Prompt:
    def __init__(self, env: str = "click-button") -> None:
        ...
        base_dir = f"prompt/{env}/" if os.path.exists(f"prompt/{env}/") else "prompt/"
        with open(f"{base_dir}example.txt") as f:
            self.example_prompt = f.read()

        with open(f"{base_dir}first_action.txt") as f:
            self.first_action_prompt = f.read()

        with open(f"{base_dir}base.txt") as f:
            self.base_prompt = f.read()
            self.base_prompt = self.replace_regex(self.base_prompt)

        with open(f"{base_dir}initialize_plan.txt") as f:
            self.init_plan_prompt = f.read()

        with open(f"{base_dir}action.txt") as f:
            self.action_prompt = f.read()

        with open(f"{base_dir}rci_action.txt") as f:
            self.rci_action_prompt = f.read()
            self.rci_action_prompt = self.replace_regex(self.rci_action_prompt)

        with open(f"{base_dir}update_action.txt") as f:
